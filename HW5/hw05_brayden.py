...
attempt_list = [] 
maxLengthList = 3
while len(attempt_list) < maxLengthList:
    password = input("Password: ")
    attempt_list.append(password)
    def withdraw(amount, password):
        nonlocal balance
        if amount > balance and password == correct_password:
            return 'Insufficient funds'
        if input('Password') != correct_password:
            maxLengthList += 1
            return 'Incorrect Password'
        if len(maxLengthList) == 3:
            return "Your account is locked. Attempts:" + attempt_list
...
def make_joint(withdraw, old_password, new_password):
    ...
    "Your account is locked. Attempts: ['my', 'secret', 'password']"
    """
error = withdraw(0, wrong_password) 
if type(error) == str:
    return error
