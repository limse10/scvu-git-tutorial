...
def make_withdraw(balance, password):
    ...
    incorrect = []
    locked = []
    pass_storage.append(password)


    def withdraw(amount, attempt):
            if 'x' in locked:
                return "Your account is locked. Attempts: " + f"{incorrect}"
            else: 
                if attempt in pass_storage: 
                    nonlocal balance
                    if amount > balance:
...
def make_joint(withdraw, old_password, new_password):
    ...
    """
    state = [0]
    state[0] = withdraw(0, old_password)
    if type(state[0]) == str:
        return state[0]
    else: 
        def linker(amount, attempt): 
            if attempt is new_password: 
