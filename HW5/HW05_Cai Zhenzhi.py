...
def replace_leaf(t, old, new):
    ...
    queue = [new_t]
    while queue!=[]:
        if queue[0][0] == old and is_leaf(queue[0]):
            queue[0][0] = new
        if is_leaf(queue[0]):
            del queue[0]
        else:
            for temp in branches(queue[0]):
                queue.append(temp)
            del queue[0]
    return new_t

def replace_leaf2(t,old,new):
    t2 = copy_tree(t)
    ...
    for i in range(len(branches(t2))):
        if is_leaf(branches(t2)[i]):
            ...
            t2[i+1] = replace_leaf2(branches(t2)[i],old,new)
    return t2
...
def totals_tree(m):
    ...
    if is_mobile(end(left(m))):
        ...
        temp.append(tree(total_weight(end(left(m)))))
    if is_mobile(end(right(m))):
        temp.append(totals_tree(end(right(m))))
    elif is_side(right(m)):
        temp.append(tree(total_weight(end(right(m)))))
...
def make_counter():
    ...
    counter = []
    def count(string):
        nonlocal possiblestrings
        nonlocal counter
        for i, temp in enumerate(possiblestrings):
            if temp==string:
                counter[i] += 1
                return counter[i]
        possiblestrings.append(string)
        counter.append(1)
        return 1
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount,password):
        ...
        nonlocal failedpass
        if len(failedpass)==3:
            return "Your account is locked. Attempts: "+str(failedpass)
        if password != password_c:
            failedpass.append(password)
...
def make_joint(withdraw, old_password, new_password):
    ...
    "Your account is locked. Attempts: ['my', 'secret', 'password']"
    """
    password_c = old_password
    password_c2 = new_password
    temp = str(withdraw(0,old_password))
    if not temp.isdigit():
        return temp
    def new_withdraw(amount,password):
        nonlocal password_c
        nonlocal password_c2
        if password_c2 == password:
            return withdraw(amount,password_c)
        else:
...
def generate_paths(t, x):
    ...
    [[0, 2], [0, 2, 1, 2]]
    """
    queue = [[t,[]]] # list of list containing tree/path taken
    ans = []
    while queue != []:
        tree = queue[0][0]
        path = queue[0][1]
