...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, pw_attempt):
        nonlocal balance
        nonlocal pwattempt_lst
        nonlocal password 
        if len(pwattempt_lst) >= 3:
            return 'Your account is locked. Attempts: ' + str(pwattempt_lst)
        if password != pw_attempt:
            pwattempt_lst.append(pw_attempt)
...
def make_joint(withdraw, old_password, new_password):
    ...
    #this is for what if someone type in a wrong old password
