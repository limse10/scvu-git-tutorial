...
def make_fib():
    ...
    def next_fib():
        nonlocal x
        #ik the HW wants me to use non local but if i delete it it still works coz the list x is mutable
    ...
    def withdraw(amount, password_try):
        ...
        if len(x)<3:
            if password_try == password:
                ...
                x.append(password_try)
                return 'Incorrect password'
        else:
            return "Your account is locked. Attempts: " + str(x)
    return withdraw
...
def make_joint(withdraw, old_password, new_password):
    ...
    def upgraded_withdraw(amount, password_try):
    ...
    e = withdraw(0,old_password)
    if type(e)==int:
        return upgraded_withdraw
    else:
        return e
